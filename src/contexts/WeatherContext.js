...
const WeatherContextProvider = ({children})=>{
    ...
    const getLocationWeatherByUserInput = async(location)=>{
        try{
            ...
            const {data} = await axios.get(`http://api.openweathermap.org/geo/1.0/direct?q=${location}&appid=${process.env.REACT_APP_API_KEY}`);
    ...
    const getLocationByGiolocation = async(data)=>{
        ...
        try{
            ...
            if(typeof(data)==="object"){
                const location = await axios.get(`http://api.openweathermap.org/geo/1.0/reverse?lat=${(data.lat).toString()}&lon=${(data.lon).toString()}&limit=5&appid=${process.env.REACT_APP_API_KEY}`);
                console.log(location.data[0]);
                dispatch({type:"GET_LOCATION_REQUEST_SUCESS",payload:location.data[0]});
            }else{
                const location = await axios.get(`http://api.openweathermap.org/geo/1.0/direct?q=${data}&appid=${process.env.REACT_APP_API_KEY}`);
